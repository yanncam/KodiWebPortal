...
function getAllNationalities(){
 ...
 function showsize($file) {
	...
	function getDetailsEntryMovie($id){
		...
		$path = str_replace("//", "/", (str_ireplace(NAX_SAMBA_PATH, NAX_LOCAL_PATH, $data["strPath"]) . "/" . $data["strFileName"]));
		$size = showsize($path);
	...
	function getDetailsEntryTvShow($id){
		...
		for($i=0;$i<$totalSaisons;$i++)
		{
			...
			for($j=0;$j<$episodes;$j++){
				...
				$path = str_replace("//", "/", (str_ireplace(NAX_SAMBA_PATH, NAX_LOCAL_PATH, $dataEpisodes["strPath"]) . "/" . $dataEpisodes["strFileName"]));
				$size = showsize($path);
